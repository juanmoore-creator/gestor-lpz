...
import { PropertyClientCard } from '../components/ui/PropertyClientCard';
...
export default function ClientsManager() {
    ...
    const getStatusBadge = (status: string) => {
        ...
        const statuses: Record<string, React.ReactNode> = {
            ...
            'lead': <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-indigo-100 text-indigo-700 shadow-sm border border-indigo-200"><TrendingUp className="w-2.5 h-2.5" /> Potencial</span>,
            'past': <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-700 shadow-sm border border-slate-200"><History className="w-2.5 h-2.5" /> Anterior</span>,
        };
        return statuses[normalizedStatus] || <span className="px-2 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 uppercase">{status}</span>;
    };
                    ...
                    <Card key={client.id} className="group flex flex-col h-full bg-white border-slate-200 hover:border-brand/30 hover:shadow-md transition-all duration-300 rounded-2xl overflow-hidden">
                        ...
                        <div className="mt-auto border-t border-slate-100 bg-slate-50/50 p-2 flex items-center gap-2">
                            <div className="flex gap-1 flex-1">
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); navigate('/app/calendar', { state: { scheduleFor: client.name } }); }}
                    ...
                    <div className="flex flex-col h-full">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-8">
                            ...
                            <button
                                ...
                                className="w-full flex items-center gap-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl hover:bg-amber-100 transition-all group text-left"
                            >
                                <div className="p-3 bg-white text-amber-600 rounded-xl shadow-sm group-hover:scale-110 transition-transform">
                                    <FileText className="w-6 h-6" />
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-900">Notas del Cliente</h4>
                                    <p className="text-xs text-slate-500 mt-0.5">Consulta la bitácora vinculada y notas importantes.</p>
                                </div>
                                <ArrowRight className="w-5 h-5 text-amber-300 ml-auto group-hover:translate-x-1 transition-transform" />
                            </button>
                                ...
                                {selectedClient.properties.length > 0 ? (
                                    <div className="space-y-3">
                                        {selectedClient.properties.map((prop) => (
                                            <PropertyClientCard key={prop.id} property={prop} />
                                        ))}
                                    </div>
                                ) : (
                                    <div className="p-8 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl text-center">
                                        <Home className="w-8 h-8 mx-auto mb-2 text-slate-300 opacity-50" />
                                        <p className="text-xs text-slate-400">No hay propiedades asignadas a este cliente.</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <ClientActivityTimeline
                                    ...
                                    propertyIds={selectedClient.properties.map(p => p.id)}
                                />
                            </div>
                        </div>
                    </div>
